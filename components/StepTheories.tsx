...
import { CheckCircle2, ArrowRight, ArrowLeft, Lightbulb, Sparkle, Scale, X, Loader2, CheckSquare, Square, PlusCircle, BookOpen, AlertTriangle } from "lucide-react";
...
const StepTheories: React.FC<StepTheoriesProps> = ({ theories, onSelect, onBack, onLoadMore, title = "", lang }) => {
  ...
  const handleCompare = async () => {
    ...
    setShowModal(true);
    try {
        const result = await compareTheories(title, selectedForCompare, lang);
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between border-b border-slate-200 pb-6 gap-4 animate-in slide-in-from-bottom-4 duration-500">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
             ...
             <h2 className="text-2xl font-black text-slate-900">{t.theoriesTitle}</h2>
          </div>
          ...
          {title && (
             <div className="mt-4 mb-4 p-4 bg-white border border-indigo-100 rounded-2xl shadow-sm ring-1 ring-indigo-50">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <span className="text-xs font-bold text-indigo-500 block mb-1">{t.studyTitle}</span>
          ...
          return (
            ...
            >
                ...
                <button 
                    ...
                    className={`absolute top-4 ${lang === 'ar' ? 'left-4' : 'right-4'} z-20 p-2 rounded-xl transition-all ${isSelectedForCompare ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-400 hover:bg-indigo-50 hover:text-indigo-500"}`}
                    title="Compare"
                >
      ...
      {selectedForCompare.length >= 2 && (
          <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-40 animate-in slide-in-from-bottom-10 fade-in duration-300">
              ...
              >
                  ...
                  <span>{t.compareBtn} ({selectedForCompare.length})</span>
              </button>
          </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-200 relative">
                ...
                <button 
                    onClick={closeModal} 
                    className={`absolute top-4 ${lang === 'ar' ? 'left-4' : 'right-4'} bg-slate-100 p-2 rounded-full text-slate-500 hover:bg-rose-100 hover:text-rose-600 transition-colors z-10`}
                >
                    <X className="w-5 h-5" />
                </button>
                ...
                <div className="p-8 md:p-10">
                    <div className="flex items-center gap-3 mb-8 border-b border-slate-100 pb-6">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-slate-900">{t.compareModalTitle}</h2>
                            <p className="text-slate-500 font-medium">{t.compareModalSubtitle}</p>
                        </div>
                    </div>
                    ...
                    {isComparing ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-800">{t.comparingMsg}</h3>
                            <p className="text-slate-500 mt-2">{t.comparingSubMsg}</p>
                        </div>
                    ) : comparisonError ? (
                        <div className="bg-rose-50 border border-rose-200 rounded-2xl p-8 text-center">
                             ...
                             <h3 className="text-xl font-bold text-rose-900 mb-2">{t.compareErrorTitle}</h3>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100">
                                    <h4 className="font-extrabold text-emerald-800 mb-3 flex items-center gap-2">
                                        <CheckCircle2 className="w-5 h-5" /> {t.commonGround}
                                    </h4>
                                ...
                                <div className="bg-rose-50 p-6 rounded-2xl border border-rose-100">
                                    <h4 className="font-extrabold text-rose-800 mb-3 flex items-center gap-2">
                                        <Scale className="w-5 h-5" /> {t.keyDifferences}
                                    </h4>
                                    <p className="text-slate-700 leading-relaxed font-medium text-sm text-justify">
                                ...
                                {comparisonResult.analysis.map((item, idx) => (
                                    <div key={idx} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                                        ...
                                        <div className="p-6 grid md:grid-cols-2 gap-6">
                                            <div>
                                                <span className="text-xs font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-2 py-1 rounded mb-2 inline-block">{t.pros}</span>
                                                <p className="text-slate-600 text-sm leading-6 font-medium">{item.pros}</p>
                                            </div>
                                            <div>
                                                <span className="text-xs font-bold uppercase tracking-wider text-rose-600 bg-rose-50 px-2 py-1 rounded mb-2 inline-block">{t.cons}</span>
                            ...
                            <div className="bg-indigo-600 rounded-2xl p-8 text-white shadow-lg shadow-indigo-200">
                                <h3 className="text-xl font-black mb-3 flex items-center gap-2">
                                    <Sparkle className="w-5 h-5" /> {t.recommendation}
                                </h3>
